...
Rubik.prototype.init = function() {
	...
	var startRotation = function() {
		this._interval = setInterval(this._rotateRandom.bind(this), 1000);
		this._autorotatePaused = false;
		this._autorotate();
	}.bind(this);
	...
	OZ.Event.add(this._node, "mouseenter", stopRotation);
	OZ.Event.add(this._node, "mouseleave", startRotation);
	
	startRotation();
}

Rubik.prototype._initUI = function() {
    var themes = ["theme-1", "theme-2", "theme-3", "theme-4", "theme-5", "theme-6", "theme-7", "theme-8", "theme-9"];
    var body = document.body;

    var setTheme = function(themeClass) {
        body.className = ""; // Clear all classes
        if (themeClass) {
            OZ.DOM.addClass(body, themeClass);
        }
    };
    ...
    for (var i = 1; i <= 9; i++) {
        var btn = OZ.$("theme-btn-" + i);
        if (btn) { // Check if button exists
            (function(index) {
                OZ.Event.add(btn, "click", function() { 
