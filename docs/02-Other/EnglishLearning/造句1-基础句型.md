...
注意：当主句动词表示“建议、命令、要求”（如 `require`, `suggest`, `demand`, `insist`）时，后面的 `that` 从句必须使用**虚拟语气。**

- **规则**：require `that` + 主语 + (should) + `动词原形`。
- The new law requires that all fake news **be** censored.
